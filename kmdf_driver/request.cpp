/// by fanxiushu 2017-10-19
/// 使用单一请求向触摸板总线驱动发起请求，测试适合 macbook pro 2017 13寸带 multi-touch bar.  Apple SPI Device 总线驱动日期 2016/5/26， 版本 6.1.6500.0
/// 其他版本的Apple SPI Device没测试过。
...
static void appleMouseEvent(mouse_event_t* evt, void* ctx) //这个函数串行执行
{
	//
...
NTSTATUS create_reuse_request(DEV_EXT* ext)
{
	...
	apple_tp_init(appleMouseEvent, ext);
	...
	status = WdfMemoryCreate( &RequestAttributes, NonPagedPool, 'Fxsd', REPORT_BUFFER_SIZE, &ext->RequestBuffer, NULL);
	if (!NT_SUCCESS(status)) {
...
NTSTATUS StartReadReport(DEV_EXT* ext)
{
	...
	///监控
	mon_workitem* m = (mon_workitem*)ExAllocatePoolWithTag(NonPagedPool, sizeof(mon_workitem), 'FXSd');
	if (m) {
